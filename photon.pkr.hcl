source "vsphere-iso" "autogenerated_1" {
  ...
  boot_command = [
    "<esc><wait>",
    "vmlinuz initrd=initrd.img root=/dev/ram0 ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/photon-kickstart.json loglevel=3 photon.media=cdrom",
    "<enter><wait>"
  ]
  ...
  network_adapters {
    ...
    network      = "atp-internal-net10"
  }

  # http_bind_address = "10.255.254.10"
...
build {
  ...
  provisioner "file" {
    destination = "/etc/systemd/system/worker.service"
    source      = "files/worker.service"
  }
